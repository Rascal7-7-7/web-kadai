...
class LanguageDetailsEnhancer {
        ...
        codeBlocks.forEach(codeBlock => {
            ...
            copyButton.addEventListener('click', () => {
                navigator.clipboard.writeText(codeBlock.textContent).then(() => {
                    copyButton.innerHTML = this.currentLanguage === 'ja' ? '✅ コピー完了' : '✅ Copied';
                    copyButton.style.color = '#00ff00';
                    copyButton.style.borderColor = '#00ff00';
                    
                    setTimeout(() => {
                        copyButton.innerHTML = this.currentLanguage === 'ja' ? '📋 コピー' : '📋 Copy';
    ...
    addCodeExecutionLinks() {
        ...
        if (paizaLanguage) {
            ...
            // サンプルコードセクションの後に挿入
            const codeSection = document.querySelector('.detail-section:has(.code-block)');
            if (codeSection && codeSection.nextElementSibling) {
                codeSection.parentNode.insertBefore(paizaSection, codeSection.nextElementSibling);
            } else if (codeSection) {
            ...
            if (copySampleBtn) {
                copySampleBtn.addEventListener('click', () => {
                    ...
                    if (codeBlock) {
                        navigator.clipboard.writeText(codeBlock.textContent).then(() => {
                            copySampleBtn.innerHTML = this.currentLanguage === 'ja' ? '✅ コピー完了' : '✅ Copied';
                            copySampleBtn.style.color = '#00ff00';
                            copySampleBtn.style.borderColor = '#00ff00';
                            
                            setTimeout(() => {
                                copySampleBtn.innerHTML = this.currentLanguage === 'ja' ? '📋 サンプルコードをコピー' : '📋 Copy Sample Code';
    ...
    addOfficialInfo() {
        ...
        const languageInfo = {
            'java': {
                ...
                companies: ['Oracle', 'Google', 'Netflix', 'Uber', 'Airbnb', 'LinkedIn'],
                projects: ['Android OS', 'Minecraft', 'Eclipse IDE', 'Apache Tomcat', 'Spring Framework']
            },
            'javascript': {
                official: 'https://developer.mozilla.org/ja/docs/Web/JavaScript',
                companies: ['Google', 'Facebook', 'Netflix', 'Airbnb', 'Uber', 'PayPal'],
                projects: ['React', 'Node.js', 'Angular', 'Vue.js', 'Express.js']
            },
            'python': {
                official: 'https://www.python.org/',
                companies: ['Google', 'Instagram', 'Spotify', 'Dropbox', 'Pinterest', 'Reddit'],
                projects: ['Django', 'Flask', 'TensorFlow', 'PyTorch', 'Pandas']
            },
            'c': {
                official: 'https://en.cppreference.com/w/c',
                companies: ['Microsoft', 'Apple', 'Linux Foundation', 'Oracle', 'Intel', 'Samsung'],
                projects: ['Linux Kernel', 'Windows Kernel', 'Git', 'SQLite', 'Redis']
            },
            'cpp': {
                official: 'https://isocpp.org/',
                companies: ['Google', 'Microsoft', 'Adobe', 'Electronic Arts', 'Blizzard', 'Unreal'],
                projects: ['Chrome', 'Windows', 'Unreal Engine', 'MySQL', 'MongoDB']
            },
            'csharp': {
                official: 'https://docs.microsoft.com/en-us/dotnet/csharp/',
                companies: ['Microsoft', 'Unity Technologies', 'Stack Overflow', 'JetBrains', 'Xamarin'],
                projects: ['.NET Framework', 'Unity', 'Visual Studio', 'Xamarin', 'ASP.NET']
            },
            'php': {
            ...
            'ruby': {
                ...
                projects: ['Ruby on Rails', 'Sinatra', 'Jekyll', 'Metasploit', 'Homebrew']
            },
            'go': {
            ...
            'kotlin': {
                ...
                projects: ['Android Development', 'Spring Boot', 'Ktor', 'Exposed', 'KotlinNative']
            },
            'swift': {
                official: 'https://swift.org/',
                companies: ['Apple', 'Lyft', 'LinkedIn', 'Airbnb', 'Slack'],
                projects: ['iOS Apps', 'macOS Apps', 'watchOS Apps', 'tvOS Apps', 'Server-side Swift']
            },
            'rust': {
            ...
            'sql': {
                ...
                projects: ['MySQL', 'PostgreSQL', 'SQL Server', 'Oracle Database', 'SQLite']
            },
            'cobol': {
                official: 'https://www.iso.org/standard/51416.html',
                companies: ['IBM', 'Bank of America', 'Citibank', 'IRS', 'Social Security Administration'],
                projects: ['Banking Systems', 'Insurance Systems', 'Government Systems', 'Legacy Enterprise Systems']
            },
            'visualbasic': {
                official: 'https://docs.microsoft.com/en-us/dotnet/visual-basic/',
                companies: ['Microsoft', 'IBM', 'HP', 'Dell', 'Intel'],
                projects: ['Windows Applications', 'Office Macros', 'Legacy Enterprise Systems', 'Educational Software']
            }
        };
        ...
        if (info) {
            ...
            officialSection.innerHTML = `
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <a href="${info.official}" target="_blank" style="color: #00ff41; text-decoration: none;">
                                ${info.official}
                            </a>
                        </div>
                    </div>
                    <div class="info-item">
                        ...
                        <div class="info-value">${info.companies.join(', ')}</div>
                    </div>
                </div>
                <div style="margin-top: 2rem;">
                    ...
                    <div class="uses-grid">
                        ${info.projects.map(project => `<div class="use-item">${project}</div>`).join('')}
                    </div>
                </div>
            `;

            // 人気度セクションの後に挿入
            const popularitySection = document.querySelector('.detail-section:last-of-type');
            if (popularitySection) {
                popularitySection.parentNode.insertBefore(officialSection, popularitySection.nextElementSibling);
            }
        }
    }
    ...
    setupLanguageSwitching() {
        ...
        if (langToggle) {
            langToggle.textContent = this.currentLanguage === 'ja' ? '🌐 EN' : '🌐 JP';
    ...
    applyCurrentLanguage() {
        ...
        document.querySelectorAll('[data-ja][data-en]').forEach(element => {
            element.textContent = this.currentLanguage === 'ja' ? element.dataset.ja : element.dataset.en;
        });
        ...
        if (searchInput) {
            searchInput.placeholder = this.currentLanguage === 'ja' ? '言語を検索...' : 'Search languages...';
        }

        // Update header text
        const headerTitle = document.querySelector('.logo-text h1');
        if (headerTitle) {
            headerTitle.textContent = this.currentLanguage === 'ja' ? 'プログラミング言語図鑑' : 'Programming Language Encyclopedia';
        }
    }
    ...
    updateNavigationMenu() {
        ...
        if (navHeader) {
            navHeader.textContent = this.currentLanguage === 'ja' ? 'メニュー' : 'Menu';
        }

        // Update navigation links
        document.querySelectorAll('.nav-links a').forEach(link => {
            if (link.textContent.includes('ホーム') || link.textContent.includes('Home')) {
                link.textContent = this.currentLanguage === 'ja' ? '🏠 ホーム' : '🏠 Home';
            }
            if (link.textContent.includes('カテゴリ') || link.textContent.includes('Categories')) {
                link.textContent = this.currentLanguage === 'ja' ? '📂 カテゴリ' : '📂 Categories';
            }
            if (link.textContent.includes('ハッカー体験') || link.textContent.includes('Hacker Experience')) {
                link.textContent = this.currentLanguage === 'ja' ? '💀 ハッカー体験' : '💀 Hacker Experience';
            }
            if (link.textContent.includes('このサイト') || link.textContent.includes('About')) {
                link.textContent = this.currentLanguage === 'ja' ? 'ℹ️ このサイトについて' : 'ℹ️ About';
            }
            if (link.textContent.includes('言語図鑑に戻る') || link.textContent.includes('Back to Encyclopedia')) {
                link.textContent = this.currentLanguage === 'ja' ? '🏠 言語図鑑に戻る' : '🏠 Back to Encyclopedia';
            }
        });
        ...
        if (navCategoriesHeader) {
            navCategoriesHeader.textContent = this.currentLanguage === 'ja' ? 'カテゴリ別に探索' : 'Explore by Category';
        }

        // Update category links
        const categoryTitles = {
            'app-development': this.currentLanguage === 'ja' ? 'アプリ開発' : 'App Development',
            'server-side': this.currentLanguage === 'ja' ? 'サーバーサイド' : 'Server-side',
            'web-development': this.currentLanguage === 'ja' ? 'Web開発' : 'Web Development',
    ...
    fixJavaScriptInteractivity() {
        // Fix JavaScript color change functionality
        const currentPage = window.location.pathname.split('/').pop().replace('.html', '');
        
        if (currentPage === 'javascript') {
            // Add event listeners for the interactive demo buttons
            const showAlertBtn = document.querySelector('button[onclick="showAlert()"]');
            const changeColorBtn = document.querySelector('button[onclick="changeColor()"]');
            const getTimeBtn = document.querySelector('button[onclick="getCurrentTime()"]');
            
            if (showAlertBtn) {
                showAlertBtn.onclick = function() {
                    alert(this.currentLanguage === 'ja' ? 'JavaScriptが動作しています！' : 'JavaScript is working!');
                }.bind(this);
            }
            ...
            if (changeColorBtn) {
                changeColorBtn.onclick = function() {
                    ...
                    if (demoOutput) {
                        demoOutput.textContent = this.currentLanguage === 'ja' 
            ...
            if (getTimeBtn) {
                getTimeBtn.onclick = function() {
                    const now = new Date();
                    const timeString = now.toLocaleString(this.currentLanguage === 'ja' ? 'ja-JP' : 'en-US');
                    const demoOutput = document.getElementById('demo-output');
                    if (demoOutput) {
                        demoOutput.textContent = this.currentLanguage === 'ja' 
                            ? `現在時刻: ${timeString}` 
